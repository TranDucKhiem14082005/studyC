...
// Hàm in ma tr?n
void inMang(int a[MAX_CITIES][MAX_CITIES], int n) {
    printf("Ma tr?n chi phí (%d x %d):\n", n, n);
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < n; j++) {
            printf("%d ", a[i][j]);
        }
...
void find_tour(const char *output_filename) {
    int total_cost = 0;
    ...
    if (output_file == NULL) {
        printf("Không th? m? file %s d? ghi k?t qu?.\n", output_filename);
        return;
    }

    fprintf(output_file, "Hành trình: TP1 -> "); // In thành ph? 1 là di?m xu?t phát
    ...
    for (int i = 1; i < n; i++) {
        ...
        if (next_city == -1) {
            fprintf(output_file, "Không tìm th?y hành trình h?p l?.\n");
            fclose(output_file);
            return;
        }
    ...
    fprintf(output_file, "T?ng chi phí: %d\n", total_cost); // In chi phí t?ng
    fclose(output_file);
}

int main() {
    // Ð?c d? li?u t? file
    readFromFile("dulich.inp");
